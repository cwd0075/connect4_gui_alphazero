...
const mcts = new m.MCTS(connect4, args, session);
...
async function aiTurn() {
  ...
  const encodedstate = connect4.get_encoded_state(neutral_state);

  const c = connect4.flatten3DArray(encodedstate);
  ...
  const value1 = Array.from(results["228"].data);
  console.log(value1);
  const policy1 = Array.from(results["222"].data);
  console.log(policy1);
  const policy_np1 = connect4.softmax(policy1);
  console.log(policy_np1);

  let mcts_probs = await mcts.search(neutral_state);
